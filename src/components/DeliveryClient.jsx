...
function DeliveryClient(props) {
    const [, setUser] = useContext(UserContext)
    const [deliveriesClient, setDeliveriesClient] = useState([])
    const navigate = useNavigate()
    const [page, setPage] = useState(1)
    const [error, setError] = useState('')

    useEffect(() => {
        fetchDeliveriesClient()
    }, [page])
    const fetchDeliveriesClient = async () => {
        if (page <= 0) {
            setPage(1)
        }
        ...
        const response = await fetch(`http://localhost:8080/api/delivery/my?page=${page}`, requestOptions);
    ...
    return (
        <div className="px-40">
            ...
            {deliveriesClient.length !== 0 ?
                <div className="container mx-auto p-2 antialiased">
                    {deliveriesClient.map((delivery, idx) =>
                        <div key={idx} onClick={()=>{navigate(`/delivery/${delivery["id"]}`)}}>
                ...
                <div className="text-center font-semibold">🔎 К сожалению, заказы по заданным параметрам не
                    найдены.
                    <Link className="text-green-500" to='/'> Перейти к оформлению доставки.</Link>
                </div>
            }
